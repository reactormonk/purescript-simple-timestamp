...
stripMillis = Regex.replace theRegex replacement
  where
  replacement = ".$1$2$3"

  theRegex =
    regex "\\.(\\d)(\\d)(\\d)\\d+" noFlags
...
printTimestamp (Timestamp dt) = dt # format milliDigits
  ...
  milliDigits = case millis of
    ...
    ms
      | ms `mod` 100 == 0 -> iso86011DigitMillisFormat
    ms
      | ms `mod` 10 == 0 -> iso86012DigitsMillisFormat
    _ -> iso8601LongFormat
